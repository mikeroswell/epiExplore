...
filtered_data <- IBM_v1_results |> filter(type == "proportion") |> 
  distinct(beta, threshold, .keep_all = TRUE)

plt<- (filtered_data |> mutate(beta = factor(beta)) |>
         ggplot(aes(x = threshold, y = cmpt, color = beta)) 
  + geom_point(size = 3)
  + geom_line()
  + labs(x = "proportion of infected so far to the final epidemic size"
         , y = "number of half-infectious-periods \n elapsed since the outbreak onset"
         , color = bquote(beta))
