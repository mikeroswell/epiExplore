...
setSeed <- sample(sample(1000:2000, size=100), size = 100)
rPar <- "exp" # sets duration distribution as exponential (base model)
days_counter <- 10
#run the IBM_for_v1.R for each values in setBeta
params <- expand.grid(x = setSeed, y = setBetas, KEEP.OUT.ATTRS = FALSE)
IBM_v1_results <- purrr::pmap_dfr(params, function(x,y) {
  out<-IBM_v1(setBeta = y, seed = x, popSize = popSize, tProb = tProb
              ,tMax = tMax)
  do.call(rbind, list(out$overTime, 
                      out$overEpiState,
                      out$overEpiStatelog))
})
saveEnvironment()
